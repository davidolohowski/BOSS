...
#'   Extremely large values (\(>1e20\)) are returned if the computation produces NaN, NA,
#'   or infinite results.
#'
#' @details
#' **Zero-mean GP** (\code{quad = FALSE}):
#' * Build covariance \(\;C = K_{xx} + \sigma^2_n I\) via
#'   \code{\link[BOSS]{cov_generator}}.
#' * Compute Cholesky factor \(L L^T = C\).
#' * Log-determinant: \(\log\det(C) = 2\sum\log(\mathrm{diag}(L))\).
#' * Precision \(Q = C^{-1} = \texttt{chol2inv}(L)\).
#' * Negative log-likelihood:
#'   \[
#'     \frac12\,y^T Q\,y \;-\; \log\det(Q)
#'     \;-\;\sum \log\bigl[\text{LogNormal}(length\_scale\mid prior\_l)\bigr].
#'   \]
#'
#' **GP with quadratic mean** (\code{quad = TRUE}):
#' * Construct design matrix \(X = [1,\,x,\,\mathrm{unique}(x\otimes x)]\).
#' * Build \(C = K_{xx} + \sigma^2_n I\) and its precision \(Q\).
#' * Form marginal precision \(S = X^T Q X\) and invert via Cholesky.
#' * Compute two quadratic terms:
#'   \(\tfrac12\,y^T Q\,y\) and \(\tfrac12\,y^T Q X S^{-1} X^T Q\,y\).
#' * Combine with log-determinants of \(Q\) and \(S^{-1}\) and the log-prior
